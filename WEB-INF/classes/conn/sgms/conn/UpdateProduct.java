...
public class UpdateProduct extends HttpServlet 
{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		try
		{
			ps1=con.prepareStatement("UPDATE `product` SET `businessowneremail`='"+bemailid+"',`p_type`='"+p_type+"',`p_name`='"+p_name+"',`p_mnf_by`='"+p_mnf_by+"',`p_details`='"+p_details+"',`p_price`='"+p_price+"',`mnf_date`='"+mnf_date+"',`warrenty`='"+warrenty+"',`p_origin`='"+p_origin+"',`p_quantity`='"+p_quantity+"',`p_img_name`='"+pf_name+"' WHERE `p_id`='"+p_id+"'");
